...
const simboloYoyo=fs.readFileSync('dist/assets/simbolo-yoyo.svg','utf8').replace(/<svg[^>]*>|<\/svg>/g,'');
fs.mkdirSync('dist/downloads',{recursive:true});
for(const hasPromo of [true,false]){
 for(const theme of ['classic','red','white']){
  ...
  const baseName=red?'menu-rojo-blanco':theme==='white'?'menu-blanco-rojo':'menu-clasico';
  const name=hasPromo?baseName:`${baseName}-sin-promo`;
...
console.log('Six outlined menus exported at 1080 × 1920 px (3 with promo, 3 without promo), with lightweight previews.');
