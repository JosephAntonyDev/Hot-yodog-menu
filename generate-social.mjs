...
for(const red of [false,true]){
 const bg=red?'#A9322A':'#FFF5DF',ink=red?'#FFFFFF':'#25231F',accent=red?'#FFFFFF':'#A9322A';
 ...
 if(!red)body+='<rect x="42" y="160" width="996" height="1590" rx="8" fill="none" stroke="#D9CBB5" stroke-width="2"/>';
 body+=`<g transform="translate(80 205) scale(.85)">${logo.replace(/<svg[^>]*>|<\/svg>/g,'').replaceAll('#A9322A',accent)}</g>`;
 ...
 const name=red?'menu-rojo-blanco':'menu-clasico';
...
fs.writeFileSync('dist/assets/logo-rojo-blanco.svg',logo.replaceAll('#A9322A','#FFFFFF'));
console.log('Two outlined menus exported at 1080 × 1920 px, with lightweight previews.');
